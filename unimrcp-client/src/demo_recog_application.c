 ...
 * 
 * $Id: demo_recog_application.c 1972 2013-05-14 20:20:48Z achaloyan $
 */
...
struct recog_app_channel_t {
	...
	demo_application_t* demo_app;
	System::IntPtr  clr;
};

/** Declaration of recognizer application methods */
static apt_bool_t recog_application_run(demo_application_t *demo_application, const char *profile, void* clrSession);
...
static apt_bool_t recog_application_on_terminate_event(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel);
...
static const mrcp_app_message_dispatcher_t recog_application_dispatcher = {
	...
	recog_application_on_terminate_event
};
...
static ucf::ITestCase^ getCLRS(System::IntPtr clr){
	return (ucf::ITestCase^)(((System::Runtime::InteropServices::GCHandle) clr).Target);
}

#define getCLRA() CmdRunner::APP

//static ucf::ITestApp^ getCLRA(System::IntPtr clr){
//	return (ucf::ITestApp^)(((System::Runtime::InteropServices::GCHandle) clr).Target);
//}


#define CLRLOG(_S_, _F_, ...)	\
do{\
	char __buf[1024];\
	System::String^ __ptr; \
	apr_snprintf(__buf, 1023, _F_, ##__VA_ARGS__); \
	__ptr = gcnew String(__buf); \
	_S_->
...
{
	demo_application_t *recog_application = (demo_application_t *)apr_palloc(pool,sizeof(demo_application_t));
	CLRLOG(getCLRA(), "demo_recog_application_create()")END(d);	
	getCLRA()->OnCreate(MrcpChannelMgr::getInstance());
...
static mrcp_channel_t* recog_application_channel_create(mrcp_session_t *session, void* clrSession)
{
	ucf::ITestCase^ clrS = getCLRS(System::IntPtr(clrSession));
	CLRLOG(clrS, "recog_application_channel_create()")END(d);
	mrcp_channel_t *channel;
	...
	/* create channel */
	recog_app_channel_t *recog_channel = (recog_app_channel_t *)apr_palloc(pool,sizeof(recog_app_channel_t));
	recog_channel->streaming = FALSE;
	recog_channel->audio_in = NULL;
	recog_channel->time_to_complete = 0;
	recog_channel->clr = System::IntPtr(clrSession);
	recog_channel->demo_app = (demo_application_t*)mrcp_application_session_object_get(session);
...
static apt_bool_t recog_application_run(demo_application_t *demo_application, const char *profile, void* clrSession)
{
	CLRLOG(getCLRA(), "recog_application_run() %s", profile)END(d);	

	mrcp_channel_t *channel;
	/* create session */
	mrcp_session_t *session = mrcp_application_session_create(demo_application->application,profile,clrSession);
	if(!session) {
		return FALSE;
	}
	
	/* create channel and associate all the required data */
	channel = recog_application_channel_create(session, clrSession);
	if(!channel) {
...
{
	//if(app_message == NULL){
		//CLRLOG(getCLRA(), "applicatoin end")END(d);		
		//getCLRA()->OnDestory();
		//return TRUE;
	//}
	CLRLOG(getCLRA(), "recog_application_handler() %d", app_message->message_type)END(d);
...
static apt_bool_t recog_application_on_session_update(mrcp_application_t *application, mrcp_session_t *session, mrcp_sig_status_code_e status)
{
	...
	CLRLOG(clrS, "recog_application_on_session_update() %s", status)END(d);
	/* not used in demo */
	return TRUE;
}
...
ref class MrcpChannel;

/** Handle the responses sent to channel add requests */
static apt_bool_t recog_application_on_channel_add(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel, mrcp_sig_status_code_e status)
{	
	ucf::ITestCase^ clrS = getCLRS(System::IntPtr(mrcp_application_session_object_get(session)));
	//recogAppRunner::Main clrA = getCLRA(application->clr);

	CLRLOG(clrS, "recog_application_on_channel_add() %d", status)END(d);
	if(status == MRCP_SIG_STATUS_CODE_SUCCESS) {
		//nmrcp_message_t *mrcp_message;
...
static apt_bool_t recog_application_on_channel_remove(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel, mrcp_sig_status_code_e status)
{
	...
	CLRLOG(clrS, "recog_application_on_channel_remove() %d", status)END(d);
	recog_app_channel_t *recog_channel = (recog_app_channel_t *)mrcp_application_channel_object_get(channel);
	clrS->OnChannelRemove(MrcpChannel::GetChannel(channel));
	/* terminate the demo */
	mrcp_application_session_terminate(session);
	MrcpChannel::RemoveChannel(channel);
...
static apt_bool_t recog_application_on_define_grammar(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel)
{
	...
	CLRLOG(clrS, "recog_application_on_define_grammar()")END(d);
...
static apt_bool_t recog_application_on_terminate_event(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel){
	ucf::ITestCase^ clrS = getCLRS(System::IntPtr(mrcp_application_session_object_get(session)));
	CLRLOG(clrS, "recog_application_on_terminate_event()", channel)END(d);	
	return FALSE;
}
...
static apt_bool_t recog_application_on_message_receive(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel, mrcp_message_t *message)
{
	ucf::ITestCase^ clrS = getCLRS(System::IntPtr(mrcp_application_session_object_get(session)));
	//recogAppRunner::Main clrA = getCLRA(application->clr);
	CLRLOG(clrS, "recog_application_on_message_receive() %d", message->start_line.message_type)END(d);

#if 0//we now process messages in test case 
	if(message->start_line.message_type == MRCP_MESSAGE_TYPE_RESPONSE) {
		CLRLOG(clrS, "new response(%d)", message->start_line.method_id)END(d);
		/* received MRCP response */
		if(message->start_line.method_id == RECOGNIZER_DEFINE_GRAMMAR) {
			CLRLOG(clrS, "RECOGNIZER_DEFINE_GRAMMAR responsed() %d", message->start_line.request_state)END(d);
			...
			CLRLOG(clrS, "RECOGNIZER_RECOGNIZE responsed() %d", message->start_line.request_state)END(d);
		...
		CLRLOG(clrS, "new event() %d", message->start_line.method_id)END(d);
		if(message->start_line.method_id == RECOGNIZER_RECOGNITION_COMPLETE) {
			CLRLOG(clrS, "RECOGNIZER_RECOGNITION_COMPLETE(%d)", message->start_line.status_code)END(d);
			...
			CLRLOG(clrS, "RECOGNIZER_START_OF_INPUT() %d", message->start_line.status_code)END(d);
		}else if(message->start_line.method_id == RECOGNIZER_RECOGNITION_INTERMEDIA_RESULT){
			CLRLOG(clrS, "RECOGNIZER_RECOGNITION_INTERMEDIA_RESULT(%d)", message->start_line.request_state)END(d);
		}		
	}
#endif
	clrS->OnMessageReceive(
		MrcpChannel::GetChannel(channel)
		,gcnew MrcpMessage(message, mrcp_application_dir_layout_get(application)));
...
static apt_bool_t recog_app_stream_destroy(mpf_audio_stream_t *stream)
{
	...
	CLRLOG(clrS, "recog_app_stream_destroy() %p", stream)END(d);
	return TRUE;
}
...
static apt_bool_t recog_app_stream_open(mpf_audio_stream_t *stream, mpf_codec_t *codec)
{
	recog_app_channel_t *recog_channel = (recog_app_channel_t *)stream->obj;
	ucf::ITestCase^ clrS = getCLRS(recog_channel->clr);
	CLRLOG(clrS, "recog_app_stream_open() %p", stream)END(d);
	return TRUE;
}
...
static apt_bool_t recog_app_stream_close(mpf_audio_stream_t *stream)
{
	recog_app_channel_t *recog_channel = (recog_app_channel_t *)stream->obj;
	ucf::ITestCase^ clrS = getCLRS(recog_channel->clr);
	CLRLOG(clrS, "recog_app_stream_close() %p", stream)END(d);
	return TRUE;
}
