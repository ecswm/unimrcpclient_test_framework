...
apt_bool_t demo_framework_app_run(demo_framework_t *framework, const char *app_name, const char *profile_name, void* clrSession)
{
	...
	if(task_msg) {
		...
		framework_task_data->clrSession = clrSession;
...
static apt_bool_t demo_framework_console_msg_process(demo_framework_t *framework, const char *app_name, const char *profile_name, void* clrSession)
{
	...
	return demo_application->run(demo_application,profile_name, clrSession);
}
	...
	if(msg->type == TASK_MSG_USER) {
		...
		switch(msg->sub_type) {
			...
			case DEMO_CONSOLE_MSG_ID:
			{
				apt_consumer_task_t *consumer_task = apt_task_object_get(task);
				demo_framework_t *framework = apt_consumer_task_object_get(consumer_task);
				demo_framework_console_msg_process(framework,data->app_name,data->profile_name, data->clrSession);
				break;
			}
		}
	}
...
static apt_bool_t demo_framework_consumer_task_create(demo_framework_t *framework)
{
	...
	if(vtable) {
		...
		vtable->on_terminate_request = demo_framework_on_terminate_request;
	}
