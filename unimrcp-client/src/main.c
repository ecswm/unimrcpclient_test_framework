...
static System::String^ newStr(void* s){	
	return gcnew String((char*) s);
}
	...
	while((rv = apr_getopt_long(opt, opt_option, &optch, &optarg)) == APR_SUCCESS) {
		switch(optch) {
			...
			case 'h':
				...
				return FALSE;
			case 'a':
				CmdRunner::APP = PluginsMgr::MainRunner::Init(MrcpChannelMgr::getInstance(), newStr((void*)optarg));
