...
static void __start_recording(){
	MicAudioProvider::Program::Start();
}

static void __stop_record(){
	MicAudioProvider::Program::Stop();
}

static int __read_record(void* buf, int size){
	return MicAudioProvider::Program::read(buf, size);
}

static void __utf82GB2312(char* in, int len, char* out){
	MicAudioProvider::Program::utf82gb2312((void*)in, (void*)out, len);
}

static void __save_result(char* in, int len){
	MicAudioProvider::Program::saveResult((void*)in,len);
}
...
static apt_bool_t recog_application_on_terminate(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel);
...
static const mrcp_app_message_dispatcher_t recog_application_dispatcher = {
	...
	recog_application_on_terminate,
};
...
static apt_bool_t recog_application_on_channel_add(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel, mrcp_sig_status_code_e status)
{
	if(status == MRCP_SIG_STATUS_CODE_SUCCESS) {
		...
		mrcp_message = demo_recognize_message_create(session, channel, dir_layout);//demo_define_grammar_message_create(session,channel,dir_layout);
		if(mrcp_message) {
			...
			if(recog_channel->audio_in == NULL) {
				apr_pool_t *pool = mrcp_application_session_pool_get(session);
...
static apt_bool_t recog_application_on_message_receive(mrcp_application_t *application, mrcp_session_t *session, mrcp_channel_t *channel, mrcp_message_t *message)
{
	...
	if(message->start_line.message_type == MRCP_MESSAGE_TYPE_RESPONSE) {
		...
		if(message->start_line.method_id == RECOGNIZER_RECOGNITION_COMPLETE) {
			...
			|| message->start_line.method_id == RECOGNIZER_INTERPRETATION_COMPLETE){
			if(message->body.length > 0){
				char* out = (char*)malloc((message->body.length + 1)* 3);
				memset(out, 0, (message->body.length + 1) * 3);
				__utf82GB2312(message->body.buf, message->body.length, out);
				apt_log(APT_LOG_MARK,APT_PRIO_INFO, "message body:%s", out);
				free(out);
				__save_result(message->body.buf, message->body.length);
			}
		}
	}
	return TRUE;
...
static apt_bool_t recog_app_stream_read(mpf_audio_stream_t *stream, mpf_frame_t *frame)
{
	...
	if(recog_channel && recog_channel->streaming == TRUE) {
		if(recog_channel->audio_in) {
			...
			int red = __read_record(frame->codec_frame.buffer, frame->codec_frame.size);
			//apt_log(APT_LOG_MARK,APT_PRIO_INFO, " read %d bytes from mic", red);
			if(red == frame->codec_frame.size){
				/* normal read */
				...
				if(0){
					...
					recog_channel->rewind_count++;
					if(recog_channel->rewind_count < 5000){
						fseek(recog_channel->audio_in, 0, SEEK_SET);
						fread(frame->codec_frame.buffer,1,frame->codec_frame.size,recog_channel->audio_in);
						frame->type |= MEDIA_FRAME_TYPE_AUDIO;
					}else{
						/* file is over */
						recog_channel->streaming = FALSE;
